...
<body>
        ...
        <div class="container">
            ...
            <div class="menu text-right">
                <ul>
                    ...
                    <li>
                        <a href="outletpage.php" class="notification">Back</a>
                    </li>
                </ul>
            </div>
    ...
    <!-- Outlet informations Section Starts Here -->
...
$db = mysqli_select_db($connection,'mainproject');
 ...
 if ($row = mysqli_fetch_array($query_run))
  {
    ?>
                                     <h3 class="text-left">Recently Posted Food by <a href="donordetailsfordonee.php?donor_ID=<?=$row['donor_ID']?>"> <?php echo $row['outletname']; ?> </a></h3>
                                     <?php } }?>
        ...
        //$result=$conn->prepare("SELECT c.* , p.* FROM donors c,excessfood p WHERE c.donor_ID=p.donor_ID AND c.donor_ID=p.donor_ID order by postedtime DESC");
        $result=$conn->prepare("SELECT donors.*,excessfood.* FROM donors INNER JOIN excessfood ON donors.username=excessfood.username order by postedtime DESC");
        $result->execute();
        $row = $result->fetch();
        //var_dump($row);
...
<?php
            ...
            echo "<a href=order.php?excessfood_ID=$row[excessfood_ID] class=btn btn-success>";
            echo "Book</a>";
